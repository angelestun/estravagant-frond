...
const Checkout = () => {
  ...
  useEffect(() => {
    return () => {
      if (isCouponApplied && couponCode) {
        fetch(`https://extravagant-back-tidu.vercel.app/api/coupons/release/${couponCode}?userId=${userData}`, {
          method: 'DELETE'
        }).catch(error => {
  ...
  const handleCouponApply = async () => {
    try {
      const response = await fetch(
        `https://extravagant-back-tidu.vercel.app/api/coupons/${couponCode}?subtotal=${subtotal}&userId=${userData}`
      );
  ...
  const handleCreateOrder = async (paypalOrderId, finalTotal, discounts) => {
    ...
    try {
      const response = await fetch('https://extravagant-back-tidu.vercel.app/api/create-order', {
        method: 'POST',
        headers: {
          ...
          onApprove={async (data, actions) => {
            try {
              ...
              if (response && response.orderId) {
                ...
                try {
                  if (couponCode) {
                    ...
                    const moveResponse = await fetch('https://extravagant-back-tidu.vercel.app/api/coupons/move-to-used', {
                      method: 'POST',
                      headers: {
                        'Content-Type': 'application/json'
                      },
                  ...
                  try {
                    ...
                    const cartClearResponse = await fetch(`https://extravagant-back-tidu.vercel.app/carrito/clear/${userData}`, {
                      method: 'DELETE'
                    });
              ...
              if (couponCode) {
                try {
                  await fetch(
                    `https://extravagant-back-tidu.vercel.app/api/coupons/release/${couponCode}?userId=${userData}`,
                    { method: 'DELETE' }
                  );
                } catch (releaseError) {
