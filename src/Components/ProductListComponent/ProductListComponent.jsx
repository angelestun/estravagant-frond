  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const response = await fetch('https://extravagant-back.vercel.app/productos/all'); 
      ...
      <div className="product-list">
        {products.filter(product => product.Stock > 0).map((product) => (
          <div key={product.ID_Producto} className="product-card">
            ...
            <img src={`https://extravagant-back.vercel.app/uploads/products/${product.Imagen}`} alt={product.Nombre_Producto} className="product-image" />
