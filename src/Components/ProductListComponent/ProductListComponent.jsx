  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const response = await fetch('https://extravagant-back-1.onrender.com/productos/all'); 
      ...
      <div className="product-list">
        {products.filter(product => product.Stock > 0).map((product) => (
          <div key={product.ID_Producto} className="product-card">
            ...
            <img src={`https://extravagant-back-1.onrender.com/uploads/products/${product.Imagen}`} alt={product.Nombre_Producto} className="product-image" />
